<template>
	<div class="containJiuJiuBaoYou">
		<div class="headerJiuJiuBaoYou">
		...
		<!-- 点击跳转到该商品详情页 -->
		<router-link  @click.native="addToThroughGoods(item)" tag="div" class="JiuJiuBaoYouInfo" 
			:key="index"  v-for="(item,index) in JiuJiuBaoYouData" 
	...
	methods:{
		...
		fetchData(){
			...
			setTimeout(() => {
				...
				//请求对应商家所有商品
				this.$axios.get("/api/selectJiuJiu/"+this.offset+"/6")
				.then( res =>{
					//判断是否全部加载完毕
					...
					this.JiuJiuBaoYouData =this.JiuJiuBaoYouData.concat(res.data)
					//请求完毕
					this.loading = false
					this.offset+=6
					this.loading1 = false
...
.containJiuJiuBaoYou .headerJiuJiuBaoYou{
	height:40px;
	line-height: 40px;
	width:100%;
...
.JiuJiuBaoYouInfo .nr_right span{
	margin: 5px 0 3px 20px;
	display: block;
	font-size: 13px;
}
.JiuJiuBaoYouInfo .nr_right span:first-child{
	font-size: 14px;
}
.JiuJiuBaoYouInfo .nr_right span:nth-child(2){
	color:#668B8B;
}
.JiuJiuBaoYouInfo .nr_right span:last-child{
	font-size: 15px;
	color: #FF7F00;
}
.JiuJiuBaoYouInfo .nr_right span:last-child i{
	font-size: 17px;
	font-weight: bold;
	color: #FF7F00;
	float: right;
